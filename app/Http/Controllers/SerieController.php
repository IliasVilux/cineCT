...
class SerieController extends Controller
{
    public static function store(){
        ...
        foreach($apiLinks as $link) {
            ...
            if(isset($serieJson->{'id'}) && isset($serieJson->{'first_air_date'})){
                $serieJson->{'first_air_date'} = (int)substr($serieJson->{'first_air_date'}, 0, -5);
                if (!empty($serieJson->{'genres'}) && $serieJson->{'first_air_date'} >= 1901 && $serieJson->{'first_air_date'} <= $current_year){
                    ...
                    if($serieJson->{'poster_path'}){
                        $serieJson->{'poster_path'} = 'https://image.tmdb.org/t/p/w500/'.$serieJson->{'poster_path'};
                    }
        ...
        foreach($allSeries as $serie){
            echo $serie->{'first_air_date'} .'<br>';
            echo $serie->{'poster_path'} .'<br>';
        }
        */
        return $allSeries;
